...
版本 0.2.7，云端构建号 15，Bundle ID 沿用 `app.eggplant1907.jet4469`。
...
构建 15 修复去插播误删正片：真实播放列表证明，原先被认为是插播的文件名/断点结构也可能是正常剧情，曾导致《绝命毒师》第一集由约 46:58 缩短到约 46:07。现在只删除播放列表用成对 `CUE-OUT`/`CUE-IN` 明确标记的广告区间；文件名变化、序号中断、CDN 变化和 `DISCONTINUITY` 均不再作为删除依据。没有明确广告标记时直接播放原地址，优先保证正片完整，代价是部分没有标记的插播可能无法自动跳过。
